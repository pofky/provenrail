...
type Resolved = { error?: string; status?: number; acct?: { account_id: string; active: boolean } };
...
async function resolveAccount(db: any, key: string): Promise<Resolved> {
  const lic = await verifyLicense(key, LICENSE_PUBLIC_KEY, Math.floor(Date.now() / 1000));
  if (lic.valid) {
    if (!ANCHOR_PLANS.has(String(lic.plan))) {
      // Naming the plan matters. "Invalid key" sends someone with a real, working key to check
      // for a typo that is not there, when what happened is that their tier does not include it.
      return { error: `hosted anchoring is not included in the ${lic.plan} plan`, status: 403 };
    }
