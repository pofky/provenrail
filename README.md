...
In CI, so the document exists at the moment the commits landed rather than being written
months later by someone with a reason to shade it:

```yaml
- uses: actions/checkout@v4
  with: {fetch-depth: 0}     # a shallow clone would attest to a stump; the action refuses one
- uses: pofky/provenrail@v0.3.1
  with:
    blame: "true"
    signing-key: ${{ secrets.PROVENRAIL_SIGNING_KEY }}   # optional; without it the signature is anonymous
    licence-key: ${{ secrets.PROVENRAIL_LICENCE_KEY }}   # optional; adds an independent RFC 3161 timestamp
...
generated by another tool. A pack name that does not exist refuses to enforce and says so on
stderr, rather than silently arming nothing, and so does a `--use` that names no pack at all.
`pr --version` prints the installed version.

### If `pr` runs the wrong program

`pr` is also a POSIX utility, the coreutils paginator. On most systems the install directory
comes first on `PATH` and ours wins, but on Windows under Git Bash it does not: `pr --version`
prints `pr (GNU coreutils) 8.32` and `pr verify --pin ...` fails with `unknown option`. If you
see that, use the module entry point, which nothing can shadow:

```
python -m provenrail verify bundle.json --pin pin.json
```

Every subcommand is identical.

### Exit codes, for CI

`pr verify` (and `pr-verify`) exits `0` when the record verifies. It exits `1` when it reached
a verdict that is not a pass: `TAMPERING DETECTED`, `NOT CONFIRMED`, `NO RECORDS`, or
`NOT A PROVENRAIL BUNDLE` (valid JSON, but not a format this verifier recognizes). It exits `2`
when it never reached a verdict at all, because the file could not be read as JSON (not UTF-8,
empty, truncated) or the arguments were wrong; that case prints `RESULT: NOT A BUNDLE`. Gate
