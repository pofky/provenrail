...
# `datetime.UTC` arrived in Python 3.11, and this module is vendored into an engine that runs
# under whatever `python3` the machine happens to have: the plugin's shim found 3.9 on macOS and
# the import failed, which took the whole guard down to "allowing" on every tool call. The alias
# is the same object, so nothing else changes.
try:
    from datetime import UTC
except ImportError:                               # pragma: no cover - Python < 3.11
    from datetime import timezone as _timezone
    UTC = _timezone.utc                           # noqa: UP017 - the 3.11 spelling is above
...
def reliable(cost: dict[str, Any]) -> bool:
    """True when a `cost_for` result is a real estimate rather than a known floor.

    Two different conditions produce a number that a budget must not treat as the truth: no rate
    at all (`priced=False`, which estimates $0.00 and so never crosses any cap), and an
    introductory rate whose published end date has passed with no successor in the table, which
    undercharges every call while still reporting itself priced. Both mean the same thing to a
    spend cap, and both callers that ask the question (`policy._estimate_cost` for a model call
    and `transcript.accrue` for a transcript line) have to answer it identically, or the same
    model would be counted by one and waved through by the other.
    """
    return bool(cost.get("priced")) and not cost.get("price_expired")
