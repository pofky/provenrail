...
_No active work in the repo._ 0.5.0 is built, proven against the artifact and tagged, but NOT
published: the PyPI upload and the Polar product switch are both blocked for the agent and are
written up in `Marketing/release-0.5.0-operator-steps.txt` and
`Marketing/polar-migration-operator-steps.txt`. The Polar one is first, because the pricing page
says $9 while the account page and live checkout still say and charge $29.

The plugin now passes `claude plugin validate`, which it never had, so the community marketplace
submission is possible for the first time; the copy is in
`Marketing/plugin-directory-submission.txt`. **After those three, the one thing left is still
posting.**
...
|------|---------|-------------|
| 2026-09-16 | provenrail | 0.5.0, built on what auto mode does NOT do. Anthropic's auto mode became the interactive default and gives away most of the destructive-delete protection for free, so the guard was repositioned onto the two gaps it leaves: no dollar cap exists anywhere in the permission system, and the classifier is absent from `claude -p`, the Agent SDK, Bedrock, Google Cloud's Agent Platform, Microsoft Foundry and Claude Platform on AWS, where hooks still fire. New `pr report` reads the transcripts already on the machine and prices them: 942 sessions, 23 projects, 31 days, 79,883 tool calls, an estimated $7,863.51, 517 calls the rules would have stopped. That number leads the homepage and every launch draft. Two ways the spend cap lied were fixed: it bound only on an SDK `model_call` a coding agent never emits, and `pr guard budget` disarmed all 44 destructive rules while reporting the cap armed. `pr guard install` wrote nothing at all in a fresh project. One policy now answers five hosts (`hosts.py`). Three model rates were wrong against the published page. Found by the validator, not by us: `hooks/hooks.json` declared its events at the top level, which loads and fires but fails `claude plugin validate`, so the marketplace listing was impossible rather than pending, through five releases. `ruff check .` had never been run in CI (it lints `src`), and all 49 errors it finds were in the file the plugin executes. Both AI Act pages had run a countdown past its own date since 2 August, showing "applies in" above "is now in force", because each page carried its own copy of the script. Verified: 2116 tests, ruff clean, 32 flows against the built wheel including a proven failure, `claude plugin validate`, a real `claude -p` session refused at `git reset --hard`, and 20 live routes swept at 320 and 375 px with no horizontal scroll, no console error and the verifier reaching both verdicts. |
| 2026-09-07 | provenrail | 0.4.3, verification sweep and a speed regression I had introduced. Widening the hook matcher to `*` put a two-process version probe in front of EVERY tool call, and PostToolUse is `*` too: 430 ms per tool call for a decision that takes single digits. Now one `--version` probe, the resolved engine cached for a day keyed by plugin version, `plugin.json` read with shell built-ins, and the bundled engine skipped on post events since it is not a recorder. Measured 115 ms bundled / 85 ms CLI, pre and post combined. Four new shim tests pin the probe count, the vanished-binary fallback and both post behaviours. Also fixed the guardrails page's meta layer, which still led with `rm -rf` after the H1 moved to `git reset --hard`, and one FAQ answer that claimed recursive deletion is denied, which stopped being true when the guard started screening the target: `rm -rf ./build` is allowed and must be. Three new claims-hygiene tests hold the title, cards and JSON-LD to the page. Verified rather than asserted: every CLI flow against the published wheel, ten tampering variants all rejected, the live browser verifier and the live anchor service, `/account` plan intent surviving sign-in, and both rewritten pages at 375 px with no horizontal scroll and a clean console. 1281 tests |
