...
def test_corrupt_ledger_degrades_to_unknown_rather_than_raising(tmp_path):
    f = tmp_path / "spend.json"
    f.write_text("{ not json")
    # the file exists, so history is "known" but empty; recording must not break
    assert spend.prior_spend("a", path=f) == (0.0, 0.0, True)
